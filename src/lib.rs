...
const SPI_IOC_WR_MODE: c_ulong =
    _ioc(IOC_WRITE, SPI_IOC_MAGIC, 1, std::mem::size_of::<u8>() as u32) as c_ulong;
const SPI_IOC_WR_BITS_PER_WORD: c_ulong =
    _ioc(IOC_WRITE, SPI_IOC_MAGIC, 3, std::mem::size_of::<u8>() as u32) as c_ulong;
const SPI_IOC_WR_MAX_SPEED_HZ: c_ulong =
    _ioc(IOC_WRITE, SPI_IOC_MAGIC, 4, std::mem::size_of::<u32>() as u32) as c_ulong;
...
fn map_pin(pin: u8) -> PyResult<u8> {
    unsafe {
        match PIN_MODE {
            ...
            Some(Mode::BOARD) => {
                let idx = pin.checked_sub(1)
                             .ok_or_else(|| PyOSError::new_err("Board pin out of range"))?
                             as usize;
...
fn spi_ioc_message(n: u32) -> u64 {
    _ioc(IOC_WRITE, SPI_IOC_MAGIC, 0,
         std::mem::size_of::<SpiIocTransfer>() as u32 * n)
}
...
impl MCP3008 {
    ...
    fn new(bus: u8, cs: u8, channel: u8, speed_khz: u32) -> PyResult<Self> {
        ...
        unsafe {
            if libc::ioctl(fd, SPI_IOC_WR_MODE, &0u8) < 0 {
                return Err(PyOSError::new_err("SPI mode set failed"));
            }
            if libc::ioctl(fd, SPI_IOC_WR_BITS_PER_WORD, &8u8) < 0 {
    ...
    fn read_raw(&self) -> PyResult<u16> {
        let tx = [0x01, ((0x08 | self.channel) << 4) as u8, 0x00];
        let mut rx = [0u8;3];
...
fn configure_serial(fd: RawFd, baud: u32) -> Result<(), String> {
    unsafe {
        ...
        if libc::tcgetattr(fd, &mut tio) != 0 {
            return Err("termios get failed".into());
        }
        ...
        if libc::tcsetattr(fd, libc::TCSANOW, &tio) != 0 {
            return Err("termios set failed".into());
        }
    }
    Ok(())
}
...
impl Serial485 {
    ...
    fn write(&self, data: &[u8]) -> PyResult<usize> {
        ...
        if n < 0 {
            return Err(PyOSError::new_err("write failed"));
        }
    ...
    fn read(&self, size: usize) -> PyResult<Vec<u8>> {
        ...
        if n < 0 {
            return Err(PyOSError::new_err("read failed"));
        }
