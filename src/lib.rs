...
use libc::{
    ...
    termios, tcgetattr, tcsetattr, cfmakeraw, cfsetispeed, cfsetospeed,
    tcdrain, tcflush, read, write, ioctl,
...
const fn _ioc(dir: u32, type_: u8, nr: u8, size: u32) -> u64 {
    ...
    ((dir << IOC_DIRSHIFT) | ((type_ as u32) << IOC_TYPESHIFT) | ((nr as u32) << IOC_NRSHIFT) | (size << IOC_SIZESHIFT)) as u64
}
...
#[pyclass]
struct MCP3008 { fd: i32, channel: u8 }
...
impl MCP3008 {
    ...
    fn new(bus: u8, cs: u8, channel: u8, speed_khz: u32) -> PyResult<Self> {
        ...
        let file = OpenOptions::new().read(true).write(true).open(&dev).map_err(|e| PyOSError::new_err(e.to_string()))?;
        let fd = file.as_raw_fd();
        unsafe {
            let m: u8 = 0;
            if libc::ioctl(fd, SPI_IOC_WR_MODE as _, &m) < 0 { return Err(PyOSError::new_err("SPI mode set failed")); }
            let b: u8 = 8;
            if libc::ioctl(fd, SPI_IOC_WR_BITS as _, &b) < 0 { return Err(PyOSError::new_err("SPI bits set failed")); }
            let s = speed_khz * 1000;
            if libc::ioctl(fd, SPI_IOC_WR_SPEED as _, &s) < 0 { return Err(PyOSError::new_err("SPI speed set failed")); }
        }
...
// RS485 Serial
fn configure_serial(fd: i32, baud: u32) -> Result<(), String> {
    unsafe {
        let mut tio: termios = std::mem::zeroed();
        if tcgetattr(fd, &mut tio)!=0 { return Err("tcgetattr failed".into()); }
        cfmakeraw(&mut tio);
        let sp = match baud {9600=>B9600,38400=>B38400,115200=>B115200,_=>return Err("Unsupported baud".into())};
        cfsetispeed(&mut tio, sp); cfsetospeed(&mut tio, sp);
        if tcsetattr(fd,TCSANOW,&tio)!=0 { return Err("tcsetattr failed".into()); }
    }
    Ok(())
}

#[pyclass]
struct Serial485 { fd: i32, de_pin: u8 }
...
impl Serial485 {
    ...
    fn new(path:String,baud:u32,timeout:f64,de_pin:u8)->PyResult<Self>{
        ...
        let file=OpenOptions::new().read(true).write(true).open(&path).map_err(|e|PyOSError::new_err(e.to_string()))?;
        let fd=file.as_raw_fd(); configure_serial(fd,baud).map_err(|e| PyOSError::new_err(e))?;
        unsafe{ let mut tio:termios=std::mem::zeroed(); tcgetattr(fd,&mut tio); tio.c_cc[VMIN]=0; tio.c_cc[VTIME]=(timeout*10.0) as u8; tcsetattr(fd,TCSANOW,&tio);}        
        Ok(Serial485{fd,de_pin})
    }
    fn write(&self,data:&[u8])->PyResult<usize>{ output(self.de_pin,1)?; let n=unsafe{write(self.fd,data.as_ptr() as *const _,data.len())}; if n<0{Err(PyOSError::new_err("Serial write failed"))} else {unsafe{tcdrain(self.fd);} output(self.de_pin,0)?;Ok(n as usize)} }
    fn read(&self,size:usize)->PyResult<Vec<u8>>{ let mut buf=vec![0;size]; let n=unsafe{read(self.fd,buf.as_mut_ptr() as *mut _,size)}; if n<0{Err(PyOSError::new_err("Serial read failed"))}else{buf.truncate(n as usize);Ok(buf)} }
    fn in_waiting(&self)->PyResult<usize>{ let mut bytes=0; unsafe{ioctl(self.fd,FIONREAD,&mut bytes);} Ok(bytes as usize) }
